    ...
    ($GPIOX:ident, $gpiox:ident, $iopxenr:ident, $PXx:ident, $extigpionr:expr, [
        ...
        pub mod $gpiox {
            ...
            use super::{
                ...
                AF11, AF12, AF13, AF14, AF15, Analog,
            };
            // use super::{Edge, ExtiPin};
            ...
            impl GpioExt for $GPIOX {
                ...
                fn split(self) -> Parts {
                    ...
                    rcc.rcc_mc_ahb4ensetr.modify(|_, w| w.$iopxenr().set_bit());
            ...
            impl<MODE> OutputPin for $PXx<Output<MODE>> {
                ...
                fn set_high(&mut self) -> Result<(), Self::Error> {
                    ...
                    unsafe { (*$GPIOX::ptr()).gpiox_bsrr.write(|w| w.bits(1 << self.i)) };
                    Ok(())
                }

                fn set_low(&mut self) -> Result<(), Self::Error> {
                    // NOTE(unsafe) atomic write to a stateless register
                    unsafe { (*$GPIOX::ptr()).gpiox_bsrr.write(|w| w.bits(1 << (self.i + 16))) };
                    Ok(())
                }
            }
            ...
            impl<MODE> StatefulOutputPin for $PXx<Output<MODE>> {
                ...
                fn is_set_low(&self) -> Result<bool, Self::Error> {
                    ...
                    Ok(unsafe { (*$GPIOX::ptr()).gpiox_odr.read().bits() & (1 << self.i) == 0 })
                }
            }
            ...
            impl<MODE> InputPin for $PXx<Output<MODE>> {
                ...
                fn is_low(&self) -> Result<bool, Self::Error> {
                    // NOTE(unsafe) atomic read with no side effects
                    Ok(unsafe { (*$GPIOX::ptr()).gpiox_idr.read().bits() & (1 << self.i) == 0 })
                }
            }
            ...
            fn _set_alternate_mode (index: usize, mode: u32)
            {
                ...
                unsafe {
                    if offset2 < 32 {
                        &(*$GPIOX::ptr()).gpiox_afrl.modify(|r, w| {
                            w.bits((r.bits() & !(0b1111 << offset2)) | (mode << offset2))
                        });
                    } else {
                        let offset2 = offset2 - 32;
                        &(*$GPIOX::ptr()).gpiox_afrh.modify(|r, w| {
                            w.bits((r.bits() & !(0b1111 << offset2)) | (mode << offset2))
                        });
                    }
                    &(*$GPIOX::ptr()).gpiox_moder.modify(|r, w| {
            ...
            $(
                ...
                impl<MODE> $PXi<MODE> {
                    ...
                    pub fn into_floating_input(self) -> $PXi<Input<Floating>> {
                        let offset = 2 * $i;
                        unsafe {
                            &(*$GPIOX::ptr()).gpiox_pupdr.modify(|r, w| {
                    ...
                    pub fn into_open_drain_output(self) -> $PXi<Output<OpenDrain>> {
                        ...
                        unsafe {
                            ...
                            &(*$GPIOX::ptr()).gpiox_otyper.modify(|r, w| {
                                w.bits(r.bits() | (0b1 << $i))
                            });
                            &(*$GPIOX::ptr()).gpiox_moder.modify(|r, w| {
                                w.bits((r.bits() & !(0b11 << offset)) | (0b01 << offset))
                ...
                impl<MODE> $PXi<Output<MODE>> {
                    ...
                    pub fn set_speed(self, speed: Speed) -> Self {
                        ...
                        unsafe {
                            &(*$GPIOX::ptr()).gpiox_ospeedr.modify(|r, w| {
                ...
                impl<MODE> OutputPin for $PXi<Output<MODE>> {
                    ...
                    fn set_high(&mut self) -> Result<(), Self::Error> {
                        // NOTE(unsafe) atomic write to a stateless register
                        unsafe { (*$GPIOX::ptr()).gpiox_bsrr.write(|w| w.bits(1 << $i)) };
                        Ok(())
                    }

                    fn set_low(&mut self) -> Result<(), Self::Error> {
                        // NOTE(unsafe) atomic write to a stateless register
                        unsafe { (*$GPIOX::ptr()).gpiox_bsrr.write(|w| w.bits(1 << ($i + 16))) };
                        Ok(())
                    }
                }
                ...
                impl<MODE> StatefulOutputPin for $PXi<Output<MODE>> {
                    ...
                    fn is_set_low(&self) -> Result<bool, Self::Error> {
                        // NOTE(unsafe) atomic read with no side effects
                        Ok(unsafe { (*$GPIOX::ptr()).gpiox_odr.read().bits() & (1 << $i) == 0 })
                    }
                }
                ...
                impl<MODE> InputPin for $PXi<Output<MODE>> {
                    ...
                    fn is_low(&self) -> Result<bool, Self::Error> {
                        // NOTE(unsafe) atomic read with no side effects
                        Ok(unsafe { (*$GPIOX::ptr()).gpiox_idr.read().bits() & (1 << $i) == 0 })
                    }
                }
                ...
                //     /// Enable external interrupts from this pin.
                //     fn enable_interrupt(&mut self, exti: &mut EXTI) {
                //         exti.gpiox_imr.modify(|r, w| unsafe { w.bits(r.bits() | (1 << $i)) });
                //     }
                ...
                //     /// Clear the interrupt pending bit for this pin
                //     fn clear_interrupt_pending_bit(&mut self) {
                //         unsafe { (*EXTI::ptr()).gpiox_pr.write(|w| w.bits(1 << $i) ) };
                //     }
                // }
